  ...
  return (
    <div className="space-y-5 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="font-display text-2xl font-bold text-white">Violations</h1>
          <p className="text-slate-400 text-sm mt-0.5">Record and manage student violations</p>
        </div>
        <button
      ...
      <div className="card overflow-hidden p-0">
        {isLoading ? (
          <div className="p-8 text-center text-slate-500 text-sm">Loading violations…</div>
        ) : violations.length === 0 ? (
              ...
              <tbody>
                {violations.map(v => (
                  <tr key={v.id}>
                    <td>
                      <div>
                        <p className="font-medium text-white text-sm">{v.violation_name}</p>
                        <p className="text-xs text-slate-500">{v.officer_name}</p>
                      </div>
                    </td>
                    ...
                    <td className="hidden md:table-cell text-slate-400 text-sm">
